...
void CEditor::CallbackSaveMap(const char *pFileName, int StorageType, void *pUser)
{
	...
	if(pEditor->Save(pFileName))
	{
		...
		pEditor->m_ValidSaveFilename = StorageType == IStorage::TYPE_SAVE && pEditor->m_pFileDialogPath == pEditor->m_aFileDialogCurrentFolder;
		pEditor->m_Map.m_Modified = false;
		pEditor->ClearMapAutosaves(pFileName);
	}
...
void CEditor::RenderStatusbar(CUIRect View)
{
	...
	if(m_lAutosaveDrafts.size() > 0)
	{
		char aBuf[64];
		str_format(aBuf, sizeof(aBuf), "%d draft(s) available", m_lAutosaveDrafts.size());
		View.VSplitRight(120.0f, &View, &Button);
		UI()->DoLabel(&Button, aBuf, 10.0f, TEXTALIGN_RIGHT);
	}
...
void CEditor::OnUpdate()
{
	...
	// autosave check
	if(m_AutosaveEnabled && m_Map.m_Modified)
	{
		int64 Now = time_get();
		if(Now - m_LastAutosaveTime > (int64)AUTOSAVE_INTERVAL * time_freq() / 1000)
		{
			DoAutosave();
...
void CEditor::GenerateAutosaveFilenames(const char *pOriginalMap, char *pAutosavePath, char *pMetadataPath, int BufSize)
{
	char aTimestamp[64];
	time_t TimeNow = time(0);
	struct tm *pTm = localtime(&TimeNow);
	str_format(aTimestamp, sizeof(aTimestamp), "%04d%02d%02d_%02d%02d%02d",
		pTm->tm_year + 1900, pTm->tm_mon + 1, pTm->tm_mday,
		pTm->tm_hour, pTm->tm_min, pTm->tm_sec);

	char aMapName[128];
	if(pOriginalMap && pOriginalMap[0])
	{
		ExtractName(pOriginalMap, aMapName, sizeof(aMapName));
	}
	else
	{
		str_copy(aMapName, "untitled", sizeof(aMapName));
	}

	str_format(pAutosavePath, BufSize, "maps/autosave/%s_%s.autosave.map", aMapName, aTimestamp);
	str_format(pMetadataPath, BufSize, "maps/autosave/%s_%s.autosave.json", aMapName, aTimestamp);
}

bool CEditor::SaveAutosaveMetadata(const SAutosaveDraft &Draft)
{
	IStorage *pStorage = Kernel()->RequestInterface<IStorage>();
	if(!pStorage)
		return false;

	IOHANDLE File = pStorage->OpenFile(Draft.m_aMetadataPath, IOFLAG_WRITE, IStorage::TYPE_SAVE);
	if(!File)
		return false;

	CJsonWriter Json(File);
	Json.BeginObject();
		Json.WriteAttribute("original_map");
		Json.WriteStrValue(Draft.m_aOriginalMap);
		Json.WriteAttribute("autosave_path");
		Json.WriteStrValue(Draft.m_aAutosavePath);
		Json.WriteAttribute("timestamp");
		Json.WriteIntValue((int64_t)Draft.m_Timestamp);
		Json.WriteAttribute("is_dirty");
		Json.WriteBoolValue(Draft.m_IsDirty);
	Json.EndObject();
...
bool CEditor::LoadAutosaveMetadata(const char *pMetadataPath, SAutosaveDraft *pDraft)
{
	IStorage *pStorage = Kernel()->RequestInterface<IStorage>();
	if(!pStorage)
		return false;
	...
	void *pData = 0;
	unsigned DataSize = 0;
	if(!pStorage->ReadFile(pMetadataPath, IStorage::TYPE_SAVE, &pData, &DataSize))
		return false;

	CJsonParser Parser;
	json_value *pJson = Parser.ParseString(static_cast<const char *>(pData));
	...
	mem_zero(pDraft, sizeof(SAutosaveDraft));
	str_copy(pDraft->m_aMetadataPath, pMetadataPath, sizeof(pDraft->m_aMetadataPath));

	const json_value &rOriginalMap = (*pJson)["original_map"];
	const json_value &rAutosavePath = (*pJson)["autosave_path"];
	const json_value &rTimestamp = (*pJson)["timestamp"];
	const json_value &rIsDirty = (*pJson)["is_dirty"];

	if(rOriginalMap.type == json_string)
		str_copy(pDraft->m_aOriginalMap, rOriginalMap.u.string.ptr, sizeof(pDraft->m_aOriginalMap));

	if(rAutosavePath.type == json_string)
		str_copy(pDraft->m_aAutosavePath, rAutosavePath.u.string.ptr, sizeof(pDraft->m_aAutosavePath));

	if(rTimestamp.type == json_integer)
		pDraft->m_Timestamp = rTimestamp.u.integer;

	if(rIsDirty.type == json_boolean)
		pDraft->m_IsDirty = rIsDirty.u.boolean != 0;

	mem_free(pData);
	return true;
}

static int ListAutosaveMetadataCallback(const char *pName, int IsDir, int DirType, void *pUser)
{
	if(IsDir || !str_endswith(pName, ".autosave.json"))
		return 0;

	array<const char *> *pFiles = static_cast<array<const char *> *>(pUser);
	char *pFilename = (char *)mem_alloc(str_length(pName) + 1);
	str_copy(pFilename, pName, str_length(pName) + 1);
	pFiles->add(pFilename);
	return 0;
}

void CEditor::ScanForAutosaves()
{
	IStorage *pStorage = Kernel()->RequestInterface<IStorage>();
	if(!pStorage)
		return;

	m_lAutosaveDrafts.clear();

	array<const char *> aMetadataFiles;
	pStorage->ListDirectory(IStorage::TYPE_SAVE, "maps/autosave", ListAutosaveMetadataCallback, &aMetadataFiles);

	for(int i = 0; i < aMetadataFiles.size(); i++)
	{
		char aMetadataPath[IO_MAX_PATH_LENGTH];
		str_format(aMetadataPath, sizeof(aMetadataPath), "maps/autosave/%s", aMetadataFiles[i]);

		SAutosaveDraft Draft;
		if(LoadAutosaveMetadata(aMetadataPath, &Draft))
		{
			m_lAutosaveDrafts.add(Draft);
		}

		mem_free((void *)aMetadataFiles[i]);
	}

	std::sort(m_lAutosaveDrafts.base_ptr(), m_lAutosaveDrafts.base_ptr() + m_lAutosaveDrafts.size());

	if(m_lAutosaveDrafts.size() > 0)
	{
		char aBuf[256];
		str_format(aBuf, sizeof(aBuf), "found %d autosave draft(s)", m_lAutosaveDrafts.size());
		Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "editor", aBuf);
	}
}

void CEditor::DoAutosave()
{
	IStorage *pStorage = Kernel()->RequestInterface<IStorage>();
	if(!pStorage)
		return;

	char aAutosaveDir[IO_MAX_PATH_LENGTH];
	str_format(aAutosaveDir, sizeof(aAutosaveDir), "maps/autosave");
	pStorage->CreateFolder(aAutosaveDir, IStorage::TYPE_SAVE);

	char aAutosavePath[IO_MAX_PATH_LENGTH];
	char aMetadataPath[IO_MAX_PATH_LENGTH];
	GenerateAutosaveFilenames(m_aFileName, aAutosavePath, aMetadataPath, sizeof(aAutosavePath));

	bool MapSaved = m_Map.Save(pStorage, aAutosavePath);
	if(!MapSaved)
	{
		Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "editor", "autosave: failed to save map");
		return;
	}

	SAutosaveDraft Draft;
	str_copy(Draft.m_aOriginalMap, m_aFileName, sizeof(Draft.m_aOriginalMap));
	str_copy(Draft.m_aAutosavePath, aAutosavePath, sizeof(Draft.m_aAutosavePath));
	str_copy(Draft.m_aMetadataPath, aMetadataPath, sizeof(Draft.m_aMetadataPath));
	Draft.m_Timestamp = time(0);
	Draft.m_IsDirty = m_Map.m_Modified;

	bool MetaSaved = SaveAutosaveMetadata(Draft);
	if(!MetaSaved)
	{
		pStorage->RemoveFile(aAutosavePath, IStorage::TYPE_SAVE);
		Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "editor", "autosave: failed to save metadata, cleaned up partial map");
		return;
	}

	SAutosaveDraft VerifyDraft;
	bool MetaValid = LoadAutosaveMetadata(aMetadataPath, &VerifyDraft);
	if(!MetaValid)
	{
		pStorage->RemoveFile(aAutosavePath, IStorage::TYPE_SAVE);
		pStorage->RemoveFile(aMetadataPath, IStorage::TYPE_SAVE);
		Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "editor", "autosave: metadata verification failed, cleaned up");
		return;
	}

	char aOldActiveAutosave[IO_MAX_PATH_LENGTH];
	char aOldActiveMeta[IO_MAX_PATH_LENGTH];
	str_copy(aOldActiveAutosave, m_aActiveDraftAutosavePath, sizeof(aOldActiveAutosave));
	str_copy(aOldActiveMeta, m_aActiveDraftMetaPath, sizeof(aOldActiveMeta));

	m_aActiveDraftAutosavePath[0] = 0;
	m_aActiveDraftMetaPath[0] = 0;

	if(aOldActiveAutosave[0])
	{
		pStorage->RemoveFile(aOldActiveAutosave, IStorage::TYPE_SAVE);
		for(int i = m_lAutosaveDrafts.size() - 1; i >= 0; i--)
		{
			if(m_lAutosaveDrafts[i].m_IsActive)
				m_lAutosaveDrafts.remove_index(i);
		}
	}
	if(aOldActiveMeta[0])
		pStorage->RemoveFile(aOldActiveMeta, IStorage::TYPE_SAVE);

	if(m_aFileName[0])
	{
		array<SAutosaveDraft> aMapDrafts;
		for(int i = 0; i < m_lAutosaveDrafts.size(); i++)
		{
			if(!str_comp(m_lAutosaveDrafts[i].m_aOriginalMap, m_aFileName))
			{
				aMapDrafts.add(m_lAutosaveDrafts[i]);
			}
		}

		if(aMapDrafts.size() >= AUTOSAVE_MAX_BACKUPS_PER_MAP)
		{
			std::sort(aMapDrafts.base_ptr(), aMapDrafts.base_ptr() + aMapDrafts.size());
			for(int i = AUTOSAVE_MAX_BACKUPS_PER_MAP - 1; i < aMapDrafts.size(); i++)
			{
				pStorage->RemoveFile(aMapDrafts[i].m_aAutosavePath, IStorage::TYPE_SAVE);
				pStorage->RemoveFile(aMapDrafts[i].m_aMetadataPath, IStorage::TYPE_SAVE);
			}
		}
	}

	char aFullPath[IO_MAX_PATH_LENGTH];
	pStorage->GetCompletePath(IStorage::TYPE_SAVE, aAutosavePath, aFullPath, sizeof(aFullPath));
	char aBuf[256];
	str_format(aBuf, sizeof(aBuf), "autosaved to '%s'", aFullPath);
	Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "editor", aBuf);
}

int CEditor::RecoverAutosaveDraft(int DraftIndex)
{
	if(DraftIndex < 0 || DraftIndex >= m_lAutosaveDrafts.size())
		return 0;
	...
	for(int i = 0; i < m_lAutosaveDrafts.size(); i++)
		m_lAutosaveDrafts[i].m_IsActive = false;

	SAutosaveDraft &Draft = m_lAutosaveDrafts[DraftIndex];

	int BackupSelectedGroup = m_SelectedGroup;
	int BackupSelectedLayer = m_SelectedLayer;

	if(m_Map.Load(pStorage, Draft.m_aAutosavePath, IStorage::TYPE_SAVE))
	{
		Draft.m_IsActive = true;

		str_copy(m_aActiveDraftAutosavePath, Draft.m_aAutosavePath, sizeof(m_aActiveDraftAutosavePath));
		str_copy(m_aActiveDraftMetaPath, Draft.m_aMetadataPath, sizeof(m_aActiveDraftMetaPath));

		m_Map.m_Modified = true;
		m_SelectedGroup = clamp(BackupSelectedGroup, 0, m_Map.m_lGroups.size() - 1);
		m_SelectedLayer = 0;
		if(m_Map.m_lGroups.size() > 0 && m_SelectedGroup >= 0)
		{
			m_SelectedLayer = clamp(BackupSelectedLayer, 0, m_Map.m_lGroups[m_SelectedGroup]->m_lLayers.size() - 1);
		}

		if(Draft.m_aOriginalMap[0])
			str_copy(m_aFileName, Draft.m_aOriginalMap, sizeof(m_aFileName));
		else
			m_aFileName[0] = 0;
		...
		char aBuf[256];
		str_format(aBuf, sizeof(aBuf), "recovered from autosave: '%s'", Draft.m_aAutosavePath);
		Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "editor", aBuf);
...
void CEditor::DiscardAutosaveDraft(int DraftIndex)
{
	if(DraftIndex < 0 || DraftIndex >= m_lAutosaveDrafts.size())
		return;
	...
	bool WasActive = m_lAutosaveDrafts[DraftIndex].m_IsActive;

	pStorage->RemoveFile(m_lAutosaveDrafts[DraftIndex].m_aAutosavePath, IStorage::TYPE_SAVE);
	pStorage->RemoveFile(m_lAutosaveDrafts[DraftIndex].m_aMetadataPath, IStorage::TYPE_SAVE);

	m_lAutosaveDrafts.remove_index(DraftIndex);
	...
	Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "editor", "autosave draft discarded");
}

void CEditor::ClearMapAutosaves(const char *pOriginalMap)
{
	IStorage *pStorage = Kernel()->RequestInterface<IStorage>();
	if(!pStorage)
		return;
	...
	for(int i = m_lAutosaveDrafts.size() - 1; i >= 0; i--)
	{
		if(!str_comp(m_lAutosaveDrafts[i].m_aOriginalMap, pOriginalMap))
		{
			bool WasActive = m_lAutosaveDrafts[i].m_IsActive;
			pStorage->RemoveFile(m_lAutosaveDrafts[i].m_aAutosavePath, IStorage::TYPE_SAVE);
			pStorage->RemoveFile(m_lAutosaveDrafts[i].m_aMetadataPath, IStorage::TYPE_SAVE);
			m_lAutosaveDrafts.remove_index(i);
...
void CEditor::ClearActiveDraftFiles()
{
	IStorage *pStorage = Kernel()->RequestInterface<IStorage>();
	if(!pStorage)
		return;

	if(m_aActiveDraftAutosavePath[0])
	{
		pStorage->RemoveFile(m_aActiveDraftAutosavePath, IStorage::TYPE_SAVE);
		m_aActiveDraftAutosavePath[0] = 0;
	}
	if(m_aActiveDraftMetaPath[0])
	{
		pStorage->RemoveFile(m_aActiveDraftMetaPath, IStorage::TYPE_SAVE);
		m_aActiveDraftMetaPath[0] = 0;
	}
...
void CEditor::CallbackSaveAutosaveAs(const char *pFileName, int StorageType, void *pUser)
{
	CEditor *pEditor = static_cast<CEditor*>(pUser);
	char aBuf[1024];
	...
	{
		str_copy(pEditor->m_aFileName, pFileName, sizeof(pEditor->m_aFileName));
		pEditor->m_ValidSaveFilename = StorageType == IStorage::TYPE_SAVE;
		pEditor->m_Map.m_Modified = false;

		char aOriginalMap[IO_MAX_PATH_LENGTH];
		str_copy(aOriginalMap, pEditor->m_aFileName, sizeof(aOriginalMap));
		pEditor->ClearActiveDraftFiles();
		pEditor->ClearMapAutosaves(aOriginalMap);
	}
